...
private:
    ...
    struct {
        ...
        std::atomic<int32_t> pcba_temp{0};
        std::atomic<int32_t> pcbb_temp{0};
        std::atomic<int32_t> pcbc_temp{0};
        std::atomic<int32_t> mota_temp{0};
        std::atomic<int32_t> motb_temp{0};
    } _tpdo3;
...
public:
    ...
    void set_cmd_status(OperatingStatus status) {
        _rpdo1.cmd_opstatus.store(status);
    }
    ...
    void set_ref_torque(float value) {
        _rpdo2.ref_torque.store(std::clamp(value, -1.0f, 1.0f));
    }
    void set_ref_speed(int16_t value) { _rpdo2.ref_speed.store(value); }

    void set_ref_dcurr(float value) {
        _rpdo3.ref_dcurr.store(std::clamp(value, -1.0f, 1.0f));
    }
    void set_openloop_ref_angle(int16_t value) {
        _rpdo3.openloop_ref_angle.store(value);
    }
    void set_ref_dvolt(float value) {
        _rpdo3.ref_dvolt.store(std::clamp(value, 0.0f, 1.0f));
    }
    ...
    int32_t pcba_temp() const { return _tpdo3.pcba_temp.load(); }
    int32_t pcbb_temp() const { return _tpdo3.pcbb_temp.load(); }
    int32_t pcbc_temp() const { return _tpdo3.pcbc_temp.load(); }
    int32_t mota_temp() const { return _tpdo3.mota_temp.load(); }
    int32_t motb_temp() const { return _tpdo3.motb_temp.load(); }
