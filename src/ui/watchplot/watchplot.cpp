...
void WatchPlot::draw() {
    ImGui::Begin(_window_title.c_str(), &_opened, ImGuiWindowFlags_MenuBar);
...
void WatchPlot::_draw_menubar() {
    if (ImGui::BeginMenuBar()) {
        if (ImGui::BeginMenu("Настройка")) {
            if (ImGui::BeginMenu("Тип")) {
                if (ImGui::RadioButton("y(t)",
                                       &_mode,
                                       std::to_underlying(Mode::y_t))) {
                    _reset();
                }
                if (ImGui::RadioButton("y(x)",
                                       &_mode,
                                       std::to_underlying(Mode::y_x))) {
                    _reset();
                }
                ImGui::EndMenu();
            }
            if (ImGui::BeginMenu("Время [сек]")) {
                //ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0.6f, 0.6f));
                ImGui::PushItemWidth(160);
                if (ImGui::InputFloat("##depth",
                                      &_time_depth,
                                      1,
                                      100,
                                      "%.1f",
                    ...
                    _time_depth =
                            std::clamp(_time_depth, 0.1f, _max_time_depth);
                }
                ImGui::PopItemWidth();
                //ImGui::PopStyleVar();
                ImGui::EndMenu();
            ...
            if (ImGui::MenuItem("Экспорт в CSV", nullptr, nullptr, !_running)) {
                _export_to_csv();
            }

            if (ImGui::MenuItem("Сброс")) {
                _reset();
            }
        ...
        if (ImGui::BeginMenu("Переменные")) {
            for (size_t i = 0; i < _charts.size(); ++i) {
                if (_charts[i].on_plot) {
                    continue;
                }

                ImGui::Selectable(_charts[i].label.c_str(),
                                  false,
                                  0,
                                  ImVec2(200, 0));
        ...
        if (ImGui::BeginDragDropTarget()) {
            if (const ImGuiPayload* payload =
                        ImGui::AcceptDragDropPayload(_dnd_id.c_str())) {
        ...
        if (_running) {
            if (ImGui::MenuItem(ICON_MDI_PAUSE "Пауза     ")) {
                _log_snapshot = _server->log_service.get_log();
                _running = false;
            }
        } else {
            if (ImGui::MenuItem(ICON_MDI_PLAY "Продолжить")) {
                _running = true;
            }
        }
...
void WatchPlot::_draw_plot_yt() {
    ...
    ImPlotAxisFlags y1flags =
            ImPlotAxisFlags_AutoFit | ImPlotAxisFlags_RangeFit;
    ImPlotAxisFlags y2flags =
            ImPlotAxisFlags_AutoFit | ImPlotAxisFlags_RangeFit;
    ImPlotAxisFlags y3flags =
            ImPlotAxisFlags_AutoFit | ImPlotAxisFlags_RangeFit;
    ...
    if (ImPlot::BeginPlot("##", ImVec2(-1, -1))) {
        ...
        ImPlot::SetupAxis(ImAxis_Y2,
                          nullptr,
                          y2flags | ImPlotAxisFlags_Opposite);
        ImPlot::SetupAxis(ImAxis_Y3,
                          nullptr,
                          y3flags | ImPlotAxisFlags_Opposite);

        if (_running) {
            _now = std::chrono::duration_cast<std::chrono::microseconds>(
                           std::chrono::steady_clock::now() -
                           _server->log_service.log_start())
                           .count() /
                   1000000.0f;
        }
        ...
        for (size_t i = 0; i < _charts.size(); ++i) {
            ...
            if (_running) {
                log_buf = _server->log_service.get_log(chart.subcategory,
                                                       chart.name);
            } else {
                auto log_iter =
                        _log_snapshot.find({chart.subcategory, chart.name});
            ...
            if (chart.on_plot && size > 0) {
                ...
                size_t offset =
                        log_buf->array_one().first - log_buf->array_two().first;
                ...
                ImPlot::PlotLine(
                        ...
                        0,
                        offset,
                        sizeof(boost::geometry::model::d2::point_xy<float>));
...
void WatchPlot::_draw_plot_yx() {
    ...
    if (ImPlot::BeginPlot("##", ImVec2(-1, -1))) {
        ImPlot::SetupAxis(ImAxis_X1,
                          _p_xchart == nullptr ? "[drop here]" :
                                                 _p_xchart->label.c_str(),
                          xflags);
        ImPlot::SetupAxis(ImAxis_Y1,
                          _p_ychart == nullptr ? "[drop here]" :
                                                 _p_ychart->label.c_str(),
                          yflags);
        ...
        if (_p_xchart != nullptr && _p_ychart != nullptr) {
            ...
            if (_running) {
                xbuf = _server->log_service.get_log(_p_xchart->subcategory,
                                                    _p_xchart->name);
                ybuf = _server->log_service.get_log(_p_ychart->subcategory,
                                                    _p_ychart->name);
            } else {
                auto xbuf_iter = _log_snapshot.find(
                        {_p_xchart->subcategory, _p_xchart->name});
                auto ybuf_iter = _log_snapshot.find(
                        {_p_ychart->subcategory, _p_ychart->name});
            ...
            if (xbuf && ybuf) {
                ...
                size_t xoffset =
                        xbuf->array_one().first - xbuf->array_two().first;
                size_t yoffset =
                        ybuf->array_one().first - ybuf->array_two().first;
                ...
                if (xsize == ysize && xoffset == yoffset) {
                    ...
                    ImPlot::PlotLine(
                            ...
                            0,
                            xoffset,
                            sizeof(boost::geometry::model::d2::point_xy<
                                    float>));
                }
            }
        }
        ...
        if (_p_xchart != nullptr &&
            ImPlot::BeginDragDropSourceAxis(ImAxis_X1)) {
            ImGui::SetDragDropPayload(_dnd_id.c_str(),
                                      &_p_xchart->idx,
                                      sizeof(int));
        ...
        if (_p_ychart != nullptr &&
            ImPlot::BeginDragDropSourceAxis(ImAxis_Y1)) {
            ImGui::SetDragDropPayload(_dnd_id.c_str(),
                                      &_p_ychart->idx,
                                      sizeof(int));
...
void WatchPlot::_export_to_csv() {
    ...
    for (size_t row = 0; row < _server->log_service.log_capacity(); ++row) {
        for (const auto& buf : snapshot) {
            if (row < buf.second.size()) {
                csv << buf.second.array_one().first[row].x()
                    << buf.second.array_one().first[row].y();
            } else {
                csv << "" << "";
            }
        }
    ...
    bsclog::success("Данные успешно экспортированы в {}.", filename);
}
