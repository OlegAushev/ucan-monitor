...
private:
    std::shared_ptr<::chss::Server> server_;
public:
    DataPanel(std::shared_ptr<::chss::Server> server,
...
private:
    ...
    void draw_valve_row(const char* title, ::chss::ValvePosition position);
    void draw_fact_row(const char* title, bool active);
};
