...
const std::vector<std::string> level_prefix = {"[ trace ]",
                                               "[ debug ]",
                                               "      ",
                                               "[ ok ]",
                                               "[warn]",
...
public:
    ...
    void add_sink(std::shared_ptr<std::ostream> sink) { _sinks.insert(sink); }
...
private:
    template<typename... Args>
    void _log(level lvl,
              bsclog_fmt::format_string<Args...> fmt,
              Args&&... args) {
        ...
        std::string payload =
                bsclog_fmt::format(fmt, std::forward<Args>(args)...);
        std::string message;
        if (lvl != level::off) {
            message = bsclog_fmt::format("{} {}",
                                         level_prefix[std::to_underlying(lvl)],
                                         payload);
        } else {
            message = bsclog_fmt::format("{}", payload);
        }
...
inline auto sink_count() { return logger::instance().sink_count(); }
